<template>
    <div class="flex flex-col w-full shadow mb-4">
        <div class="flex flex-col w-full p-2 text-sm text-center">
            <p class="w-full">Status: <span class="italic">{{ info.status }}</span></p>
            <p v-if="info.status.includes('review') || info.status.includes('rejected')" class="w-full bg-red-500 text-white rounded-lg px-4 py-2">{{ info.statusMessage }}</p>
        </div>
